...
class AssemblyResult:
    ...
    def to_fasta(self, filepath: Path | None = None) -> str:
        """Export contigs to FASTA format."""
        fasta_lines = []
        for contig in self.contigs:
            fasta_lines.append(
                f">{contig.id} length={contig.length} coverage={contig.coverage:.1f}"
            )
...
class DeBruijnAssembler(Assembler):
    ...
    def __init__(self, k: int = 31, params: dict | None = None):
    ...
    def _trace_simple_path(self, start: str, max_len: int = 100) -> list[str] | None:
...
class OverlapLayoutConsensus(Assembler):
    ...
    def _build_layout(self, reads: list[str]) -> list[list[int]]:
        ...
        for read1, read2, start1, _start2, overlap_len, _identity in self.overlaps:
        ...
        for start in starts:
            ...
            while adj[current]:
                ...
                for next_read, overlap_len, _ in adj[current]:
                    if next_read not in used and overlap_len > best_score:
    ...
    def _generate_consensus(
        ...
        for layout_idx, layout in enumerate(layouts):
            ...
            for i in range(1, len(layout)):
                ...
                # Find overlap info
                overlap_info = None
                for r1, r2, s1, s2, olen, _ident in self.overlaps:
    ...
    def get_assembly_graph(self) -> "AssemblyGraph":
        ...
        nodes = list({o[0] for o in self.overlaps} | {o[1] for o in self.overlaps})
    ...
    def _map_reads(self, reads: list[str]) -> list[tuple[int, str, int]]:
        ...
        for read_idx, read in enumerate(reads):
            ...
            if seed_matches:
                ...
                matches = sum(1 for a, b in zip(read, ref_region, strict=False) if a == b)
