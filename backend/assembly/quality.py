...
class AssemblyQC:
    ...
    def _align_to_reference(
        ...
        for contig_idx, contig in enumerate(contigs):
            ...
            if matches:
                ...
                mm = 0
                for a, b in zip(contig, ref_region):
                    if a != b:
                        mm += 1
    ...
    def _detect_misassemblies(self, alignments: List[Dict]) -> int:
        ...
        for contig_idx, alns in by_contig.items():
            if len(alns) > 1:
...
class BUSCOAnalysis:
    ...
    def __init__(
        self,
        lineage_markers: Optional[Dict[str, str]] = None,
    ):
    ...
    def analyze(
        ...
        for marker_id, marker_seq in self.lineage_markers.items():
            # Search for marker
