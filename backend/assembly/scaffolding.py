    ...
    def to_agp(self) -> str:
        ...
        for comp in self.components:
            if comp.gap_before > 0:
                ...
                lines.append(f"{self.id}\t{pos}\t{end}\t{part}\tN\t{comp.gap_before}\tscaffold\tyes\tpaired-ends")
                pos = end + 1
                part += 1
            ...
            lines.append(f"{self.id}\t{pos}\t{end}\t{part}\tW\t{comp.contig_id}\t1\t{comp_len}\t{comp.orientation}")
            pos = end + 1
            part += 1
...
class ScaffoldGraph:
    ...
    def get_best_link(
        ...
        min_support: int = 2,
    ) -> Optional[ScaffoldLink]:
        """Get best link from contig end."""
        candidates = []
        for link in self.adjacency.get(contig_id, []):
            if link.contig1_id == contig_id and link.contig1_end == end:
                if link.support >= min_support:
                    candidates.append(link)
            elif link.contig2_id == contig_id and link.contig2_end == end:
                if link.support >= min_support:
                    candidates.append(link)
...
class Scaffolder:
    ...
    def _find_links_from_pairs(
        ...
        for pair_idx, mappings in contig_map.items():
            if len(mappings) != 2:
                continue
        ...
        for (c1, e1, c2, e2), gaps in link_counts.items():
            if len(gaps) >= self.min_link_support:
                self.graph.add_link(ScaffoldLink(
                    contig1_id=c1,
                    contig1_end=e1,
                    contig2_id=c2,
                    contig2_end=e2,
        ...
        for read in long_reads:
            ...
            for contig_id, positions in matches.items():
                if len(positions) >= 3:  # Minimum k-mer matches
                    ...
                    contig_start = positions[0][1]
                    contig_end = positions[-1][1] + k
    ...
    def _build_hic_contact_matrix(
        ...
        for chrom1, pos1, chrom2, pos2 in hic_contacts:
...
class GapFiller:
        ...
        for i, comp in enumerate(scaffold.components):
            ...
            if comp.gap_before > 0:
                ...
                if gap_seq:
                    ...
                    new_components.append(ScaffoldComponent(
    ...
    def _find_gap_reads(
        ...
        k = 15
        left_kmers = set(left_flank[i:i + k] for i in range(len(left_flank) - k + 1))
        right_kmers = set(right_flank[i:i + k] for i in range(len(right_flank) - k + 1))
        
        gap_reads = []
        for read in reads:
            read_upper = read.upper()
            read_kmers = set(read_upper[i:i + k] for i in range(len(read_upper) - k + 1))
    ...
    def _find_bridge_contig(
        ...
        for contig in contigs:
            ...
            left_match = any(left_flank[i:i + k] in seq 
                           for i in range(len(left_flank) - k + 1))
            right_match = any(right_flank[i:i + k] in seq 
                            for i in range(len(right_flank) - k + 1))
