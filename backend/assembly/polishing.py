...
class ConsensusPolisher:
    ...
    def polish(
        ...
        sequence: str,
        reads: List[str],
        qualities: Optional[List[List[int]]] = None,
    ) -> PolishingResult:
...
class HomopolymerCorrector:
            ...
            if left_pos != -1:
                ...
                if hp_length > 0:
                    ...
                    if hp_start + hp_length + len(right_flank) <= len(read):
                        if read[hp_start + hp_length:hp_start + hp_length + len(right_flank)] == right_flank:
    ...
    def _apply_corrections(
        ...
        for start, end, base, true_length in corrections:
            ...
            if true_length > current_length:
                # Insert bases
                extra = base * (true_length - current_length)
...
class Racon:
    ...
    def polish(
        ...
        for start in range(0, len(sequence), self.window_size // 2):
            ...
            window_alns = [
                ...
                if a['ref_start'] < end and a['ref_end'] > start
            ]
    ...
    def _merge_windows(
        ...
        for start, end, consensus in windows:
