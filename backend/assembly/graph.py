    ...
    def find_tips(self, max_length: int = 1000) -> List[str]:
        ...
        for node in self.nodes:
            ...
            if node_len <= max_length:
                if in_deg.get(node, 0) == 1 and out_deg.get(node, 0) == 0:
                    tips.append(node)
                elif in_deg.get(node, 0) == 0 and out_deg.get(node, 0) == 1:
                    tips.append(node)
...
class UnitGraph:
    ...
    def build_from_kmers(self, kmer_counts: Dict[str, int], min_count: int = 3):
        ...
        for kmer in solid_kmers:
            ...
            while True:
                extensions = [s for s in kmer_graph.get(current_suffix, set()) 
            ...
            # Calculate coverage
            cov = np.mean([kmer_counts.get(unitig[i:i + self.k], 0) 
