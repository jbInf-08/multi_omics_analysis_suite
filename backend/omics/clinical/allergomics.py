...
class AllergomicsModule(OmicsModuleBase):
    ...
    def __init__(self):
        ...
        self._pipelines = [
            Pipeline(name="allergen_analysis", description="Allergen response profiling",
                steps=["load_data", "qc", "ige_profiling", "epitope_mapping", "response_prediction"],
                default_parameters={"database": "allergen_online"}),
        ]
        self._analyses = [
            AnalysisDefinition(name="allergen_profiling", description="Profile allergen-specific responses",
                parameters={"allergens": {"type": "list", "default": None}}, output_types=["table", "heatmap"]),
            AnalysisDefinition(name="ige_repertoire", description="IgE antibody repertoire analysis",
                parameters={}, output_types=["table", "distribution"]),
            AnalysisDefinition(name="allergy_risk", description="Predict allergy risk from genetics",
                parameters={}, output_types=["table", "risk_score"]),
        ]
    
    @property
    def name(self) -> str: return "allergomics"
    @property
    def category(self) -> OmicsCategory: return OmicsCategory.CLINICAL
    @property
    def description(self) -> str: return "Allergy genomics and immune response profiling"
    
    def load_data(self, source: DataSource) -> OmicsData:
        if source.source_type == "file":
            df = pd.read_csv(source.path, sep="\t" if source.path.endswith(".tsv") else ",", index_col=0)
            return OmicsData(data=df.T, feature_names=df.index.tolist(), sample_names=df.columns.tolist(), data_type="allergomics", source=source)
    ...
    def normalize(self, data: OmicsData, method: str = "log", params: Optional[Dict[str, Any]] = None) -> OmicsData:
