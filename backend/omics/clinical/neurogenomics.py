...
class NeurogenomicsModule(OmicsModuleBase):
    ...
    def __init__(self):
        ...
        self._pipelines = [
            Pipeline(name="brain_expression", description="Brain region-specific expression analysis",
                steps=["load_data", "qc", "region_mapping", "cell_type_deconvolution", "differential_analysis"],
                default_parameters={"atlas": "allen_brain"}),
        ]
        self._analyses = [
            AnalysisDefinition(name="brain_region_expression", description="Brain region-specific expression",
                parameters={"regions": {"type": "list", "default": None}}, output_types=["table", "brain_map"]),
            AnalysisDefinition(name="neuropsych_gwas", description="Neuropsychiatric GWAS analysis",
                parameters={"traits": {"type": "list", "default": ["schizophrenia", "alzheimers"]}}, output_types=["table", "manhattan"]),
            AnalysisDefinition(name="neural_cell_types", description="Neural cell type deconvolution",
                parameters={"method": {"type": "str", "default": "music"}}, output_types=["table", "bar_chart"]),
        ]
    
    @property
    def name(self) -> str: return "neurogenomics"
    @property
    def category(self) -> OmicsCategory: return OmicsCategory.CLINICAL
    @property
    def description(self) -> str: return "Brain and neurological genomics analysis"
    
    def load_data(self, source: DataSource) -> OmicsData:
        if source.source_type == "file":
            df = pd.read_csv(source.path, sep="\t" if source.path.endswith(".tsv") else ",", index_col=0)
            return OmicsData(data=df.T, feature_names=df.index.tolist(), sample_names=df.columns.tolist(), data_type="neurogenomics", source=source)
