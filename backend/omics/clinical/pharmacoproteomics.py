"""
Pharmacoproteomics Module - Drug-protein interaction analysis
"""
...
class PharmacoproteomicsModule(OmicsModuleBase):
    """Pharmacoproteomics module for drug-protein interaction analysis."""
    ...
    def __init__(self):
        ...
        self._pipelines = [
            Pipeline(name="drug_target_analysis", description="Drug target identification and validation",
                steps=["load_data", "qc", "target_identification", "binding_analysis", "pathway_impact"],
                default_parameters={"database": "drugbank"}),
        ]
        self._analyses = [
            AnalysisDefinition(name="drug_targets", description="Identify drug target proteins",
                parameters={"database": {"type": "str", "default": "drugbank"}}, output_types=["table"]),
            AnalysisDefinition(name="drug_response", description="Analyze protein-level drug response",
                parameters={"fdr": {"type": "float", "default": 0.05}}, output_types=["table", "heatmap"]),
            AnalysisDefinition(name="off_target_effects", description="Predict off-target effects",
                parameters={}, output_types=["table"]),
        ]
    
    @property
    def name(self) -> str: return "pharmacoproteomics"
    @property
    def category(self) -> OmicsCategory: return OmicsCategory.CLINICAL
    @property
    def description(self) -> str: return "Drug-protein interactions and drug response proteomics"
    
    def load_data(self, source: DataSource) -> OmicsData:
        if source.source_type == "file":
            df = pd.read_csv(source.path, sep="\t" if source.path.endswith(".tsv") else ",", index_col=0)
            return OmicsData(data=df.T, feature_names=df.index.tolist(), sample_names=df.columns.tolist(), data_type="pharmacoproteomics", source=source)
