...
class RegulomicsModule(OmicsModuleBase):
    ...
    def __init__(self):
        ...
        self._pipelines = [
            Pipeline(name="grn_analysis", description="Gene regulatory network inference",
                steps=["load_data", "network_inference", "tf_activity", "target_prediction", "visualization"],
                default_parameters={"method": "genie3"}),
        ]
        self._analyses = [
            AnalysisDefinition(name="network_inference", description="Infer gene regulatory network",
                parameters={"method": {"type": "str", "default": "genie3"}}, output_types=["network"]),
            AnalysisDefinition(name="tf_activity", description="Transcription factor activity estimation",
                parameters={"database": {"type": "str", "default": "dorothea"}}, output_types=["table"]),
            AnalysisDefinition(name="regulon_analysis", description="Analyze TF regulons",
                parameters={}, output_types=["table", "network"]),
        ]
    
    @property
    def name(self) -> str: return "regulomics"
    @property
    def category(self) -> OmicsCategory: return OmicsCategory.INTERACTIONS
    @property
    def description(self) -> str: return "Gene regulatory network and transcription factor analysis"
    
    def load_data(self, source: DataSource) -> OmicsData:
        if source.source_type == "file":
            df = pd.read_csv(source.path, sep="\t" if source.path.endswith(".tsv") else ",", index_col=0)
            return OmicsData(data=df.T, feature_names=df.index.tolist(), sample_names=df.columns.tolist(), data_type="regulomics", source=source)
