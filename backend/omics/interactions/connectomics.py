...
class ConnectomicsModule(OmicsModuleBase):
    ...
    def __init__(self):
        ...
        self._pipelines = [
            Pipeline(name="connectome_analysis", description="Brain connectivity analysis",
                steps=["load_data", "preprocessing", "connectivity_matrix", "network_analysis", "visualization"],
                default_parameters={"parcellation": "aparc"}),
        ]
        self._analyses = [
            AnalysisDefinition(name="connectivity_matrix", description="Generate connectivity matrix",
                parameters={"threshold": {"type": "float", "default": 0.1}}, output_types=["matrix", "heatmap"]),
            AnalysisDefinition(name="graph_metrics", description="Calculate graph theory metrics",
                parameters={}, output_types=["table"]),
        ]
    
    @property
    def name(self) -> str: return "connectomics"
    @property
    def category(self) -> OmicsCategory: return OmicsCategory.INTERACTIONS
    @property
    def description(self) -> str: return "Neural connectivity and brain network analysis"
    
    def load_data(self, source: DataSource) -> OmicsData:
        if source.source_type == "file":
            df = pd.read_csv(source.path, sep="\t" if source.path.endswith(".tsv") else ",", index_col=0)
            return OmicsData(data=df, feature_names=df.columns.tolist(), sample_names=df.index.tolist(), data_type="connectomics", source=source)
