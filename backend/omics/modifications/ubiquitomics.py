...
class UbiquitomicsModule(OmicsModuleBase):
    ...
    def __init__(self):
        ...
        self._pipelines = [
            Pipeline(name="ubiquitination_analysis", description="Ubiquitination site analysis",
                steps=["load_data", "qc", "site_localization", "quantification", "degradation_analysis"],
                default_parameters={"diglycine_remnant": True}),
        ]
        self._analyses = [
            AnalysisDefinition(name="differential_ubiquitination", description="Differential ubiquitination",
                parameters={"fdr": {"type": "float", "default": 0.05}}, output_types=["table"]),
            AnalysisDefinition(name="ub_chain_analysis", description="Ubiquitin chain type analysis",
                parameters={}, output_types=["table"]),
        ]
    
    @property
    def name(self) -> str: return "ubiquitomics"
    @property
    def category(self) -> OmicsCategory: return OmicsCategory.MODIFICATIONS
    @property
    def description(self) -> str: return "Protein ubiquitination and degradation signal analysis"
    
    def load_data(self, source: DataSource) -> OmicsData:
        if source.source_type == "file":
            df = pd.read_csv(source.path, sep="\t" if source.path.endswith(".tsv") else ",", index_col=0)
            return OmicsData(data=df.T, feature_names=df.index.tolist(), sample_names=df.columns.tolist(), data_type="ubiquitomics", source=source)
