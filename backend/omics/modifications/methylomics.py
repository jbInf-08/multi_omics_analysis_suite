...
class MethylomicsModule(OmicsModuleBase):
    ...
    def __init__(self):
        ...
        self._pipelines = [
            Pipeline(name="methylation_analysis", description="Protein methylation site analysis",
                steps=["load_data", "qc", "site_identification", "quantification", "differential_analysis"],
                default_parameters={"modifications": ["K-Me", "R-Me"]}),
        ]
        self._analyses = [
            AnalysisDefinition(name="differential_methylation", description="Differential protein methylation",
                parameters={"fdr": {"type": "float", "default": 0.05}}, output_types=["table"]),
            AnalysisDefinition(name="histone_methylation", description="Histone methylation marks analysis",
                parameters={}, output_types=["table", "heatmap"]),
        ]
    
    @property
    def name(self) -> str: return "methylomics"
    @property
    def category(self) -> OmicsCategory: return OmicsCategory.MODIFICATIONS
    @property
    def description(self) -> str: return "Protein arginine and lysine methylation analysis"
    
    def load_data(self, source: DataSource) -> OmicsData:
        if source.source_type == "file":
            df = pd.read_csv(source.path, sep="\t" if source.path.endswith(".tsv") else ",", index_col=0)
            return OmicsData(data=df.T, feature_names=df.index.tolist(), sample_names=df.columns.tolist(), data_type="methylomics", source=source)
