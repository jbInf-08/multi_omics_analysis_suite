...
class AcetylomicsModule(OmicsModuleBase):
    ...
    def __init__(self):
        ...
        self._pipelines = [
            Pipeline(name="acetylation_analysis", description="Protein acetylation site analysis",
                steps=["load_data", "qc", "site_localization", "quantification", "differential_analysis"],
                default_parameters={"modification": "K-Ac"}),
        ]
        self._analyses = [
            AnalysisDefinition(name="differential_acetylation", description="Differential acetylation analysis",
                parameters={"fdr": {"type": "float", "default": 0.05}}, output_types=["table", "volcano"]),
            AnalysisDefinition(name="histone_acetylation", description="Histone-specific acetylation analysis",
                parameters={}, output_types=["table", "heatmap"]),
        ]
    
    @property
    def name(self) -> str: return "acetylomics"
    @property
    def category(self) -> OmicsCategory: return OmicsCategory.MODIFICATIONS
    @property
    def description(self) -> str: return "Protein lysine acetylation and histone modification analysis"
    
    def load_data(self, source: DataSource) -> OmicsData:
        if source.source_type == "file":
            df = pd.read_csv(source.path, sep="\t" if source.path.endswith(".tsv") else ",", index_col=0)
            return OmicsData(data=df.T, feature_names=df.index.tolist(), sample_names=df.columns.tolist(), data_type="acetylomics", source=source)
