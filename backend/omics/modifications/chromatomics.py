...
class ChromatomicsModule(OmicsModuleBase):
    ...
    def __init__(self):
        ...
        self._pipelines = [
            Pipeline(name="chromatin_state", description="Chromatin state analysis",
                steps=["load_data", "qc", "state_learning", "annotation", "differential_analysis"],
                default_parameters={"n_states": 15}),
        ]
        self._analyses = [
            AnalysisDefinition(name="chromatin_states", description="Learn chromatin states",
                parameters={"n_states": {"type": "int", "default": 15}}, output_types=["table", "heatmap"]),
            AnalysisDefinition(name="differential_accessibility", description="Differential chromatin accessibility",
                parameters={"fdr": {"type": "float", "default": 0.05}}, output_types=["table"]),
        ]
    
    @property
    def name(self) -> str: return "chromatomics"
    @property
    def category(self) -> OmicsCategory: return OmicsCategory.MODIFICATIONS
    @property
    def description(self) -> str: return "Chromatin state and accessibility analysis"
    
    def load_data(self, source: DataSource) -> OmicsData:
        if source.source_type == "file":
            df = pd.read_csv(source.path, sep="\t" if source.path.endswith(".tsv") else ",", index_col=0)
            return OmicsData(data=df.T, feature_names=df.index.tolist(), sample_names=df.columns.tolist(), data_type="chromatomics", source=source)
    ...
    def normalize(self, data: OmicsData, method: str = "rpm", params: Optional[Dict[str, Any]] = None) -> OmicsData:
