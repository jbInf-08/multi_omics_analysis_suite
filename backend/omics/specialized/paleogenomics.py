...
class PaleogenomicsModule(OmicsModuleBase):
    ...
    def visualize(self, result: AnalysisResult, params: Optional[Dict] = None) -> Visualization:
        return Visualization(name=f"{result.name}_plot", plot_type="line",
                           data=result.data.to_dict(), config=params or {})
    
    def get_available_pipelines(self) -> List[Pipeline]:
        return [
            Pipeline(name="adna_authentication", description="Ancient DNA authentication",
                    steps=["load", "damage_analysis", "contamination", "authentication"]),
            Pipeline(name="population_genetics", description="Ancient population genetics",
