...
class MicrobiomicsModule(OmicsModuleBase):
    ...
    def visualize(self, result: AnalysisResult, params: Optional[Dict] = None) -> Visualization:
        return Visualization(name=f"{result.name}_plot", plot_type="stacked_bar",
                           data=result.data.to_dict(), config=params or {})
    
    def get_available_pipelines(self) -> List[Pipeline]:
        return [
            Pipeline(name="functional_profiling", description="Microbiome functional profiling",
                    steps=["load", "qc", "normalize", "pathway_analysis", "visualization"]),
            Pipeline(name="host_microbe", description="Host-microbiome interaction analysis",
