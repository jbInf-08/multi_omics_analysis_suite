...
class OrganomicsModule(OmicsModuleBase):
    ...
    def get_available_pipelines(self) -> list[Pipeline]:
        return [
            Pipeline(
                name="organoid_profiling",
                description="Organoid characterization",
    ...
    def get_available_analyses(self) -> list[AnalysisDefinition]:
        return [
            AnalysisDefinition(
                name="maturation_analysis", description="Organoid maturation profiling"
            ),
            AnalysisDefinition(name="drug_response", description="Drug response in organoids"),
            AnalysisDefinition(
                name="cell_composition", description="Cell type composition analysis"
            ),
        ]
