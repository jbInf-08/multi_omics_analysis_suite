...
class GenomicsModule(OmicsModuleBase):
    ...
    def __init__(self):
        ...
        self._analyses = [
            AnalysisDefinition(
                ...
                parameters={
                    "min_depth": {"type": "int", "default": 10, "description": "Minimum read depth"},
                    "population": {"type": "str", "default": "gnomAD", "description": "Population database"},
                },
            ...
            AnalysisDefinition(
                ...
                parameters={
                    ...
                    "database": {"type": "str", "default": "cosmic", "description": "Mutation database"},
                },
            ...
            AnalysisDefinition(
                ...
                parameters={
                    "method": {"type": "str", "default": "MutSigCV", "description": "Detection method"},
                    "fdr_threshold": {"type": "float", "default": 0.1, "description": "FDR threshold"},
                },
            ...
            AnalysisDefinition(
                ...
                parameters={
                    "reference_panel": {"type": "str", "default": "1000G", "description": "Reference panel"},
    ...
    def _load_maf(self, file_path: Path) -> OmicsData:
        ...
        required_cols = ["Hugo_Symbol", "Chromosome", "Start_Position", "Reference_Allele", "Tumor_Seq_Allele2"]
        if not all(col in df.columns for col in required_cols):
            raise ValueError("MAF file missing required columns")
        
        feature_names = df["Hugo_Symbol"].tolist()
        sample_names = df["Tumor_Sample_Barcode"].unique().tolist() if "Tumor_Sample_Barcode" in df.columns else ["sample"]
    ...
    def preprocess(
        ...
        processed.preprocessing_history.append(f"preprocess(min_qual={min_qual}, min_depth={min_depth})")
    ...
    def quality_control(
        ...
        completeness = 1.0 - (data.data.isna().sum().sum() / data.data.size) if data.data.size > 0 else 0
    ...
    def _analyze_oncogenic_mutations(self, data: OmicsData, params: AnalysisParams) -> AnalysisResult:
