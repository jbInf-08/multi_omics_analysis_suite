...
class MetabolomicsModule(OmicsModuleBase):
    ...
    def normalize(
        self, data: OmicsData, method: str = "pqn", params: dict[str, Any] | None = None
    ) -> OmicsData:
        ...
        if method == "pqn":  # Probabilistic Quotient Normalization
            ...
            normalized.data = (normalized.data - normalized.data.mean()) / np.sqrt(
                normalized.data.std()
            )
    ...
    def _analyze_pathways(self, data: OmicsData, params: AnalysisParams) -> AnalysisResult:
        return AnalysisResult(
            analysis_type="pathway_analysis", status="success", data={"pathways": []}, summary={}
        )
