...
class ProteomicsModule(OmicsModuleBase):
    ...
    def preprocess(
        ...
        processed.feature_names = [
            f for f, k in zip(processed.feature_names, keep_proteins, strict=False) if k
        ]
    ...
    def quality_control(
        ...
        cv = (
            total_intensity.std() / total_intensity.mean()
            if total_intensity.mean() > 0
            else float("inf")
        )
        metrics.append(
            QCMetric(
    ...
    def _analyze_differential_abundance(
        ...
        if data.sample_metadata is not None and "condition" in data.sample_metadata.columns:
            ...
            if len(conditions) >= 2:
                group1 = data.sample_metadata[
                    data.sample_metadata["condition"] == conditions[0]
                ].index
                group2 = data.sample_metadata[
                    data.sample_metadata["condition"] == conditions[1]
                ].index
        ...
        if results:
            ...
            da_df["significant"] = (da_df["padj"] < fdr_threshold) & (
                abs(da_df["log2FoldChange"]) > log2fc_threshold
            )
        else:
            da_df = pd.DataFrame()
    ...
    def _analyze_pathway_enrichment(
        self, data: OmicsData, params: AnalysisParams
    ) -> AnalysisResult:
        ...
        query = {str(x) for x in top}
        background = {str(x) for x in data.feature_names}
