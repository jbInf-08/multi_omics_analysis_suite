...
class MetagenomicsModule(OmicsModuleBase):
    ...
    def preprocess(self, data: OmicsData, params: dict[str, Any] | None = None) -> OmicsData:
        ...
        params.get("min_abundance", 0.001)
    ...
    def quality_control(self, data: OmicsData, params: dict[str, Any] | None = None) -> QCReport:
        ...
        return QCReport(
            passed=passed, metrics=metrics, details={"median_depth": float(read_depths.median())}
        )

    def normalize(
        self, data: OmicsData, method: str = "relative", params: dict[str, Any] | None = None
    ) -> OmicsData:
    ...
    def _analyze_beta_diversity(self, data: OmicsData, params: AnalysisParams) -> AnalysisResult:
        ...
        dist_df = pd.DataFrame(dist_matrix, index=data.sample_names, columns=data.sample_names)
            ...
            if len(conditions) >= 2:
                ...
                for taxon in data.feature_names:
                    if taxon in data.data.columns:
                        v1, v2 = (
                            data.data.loc[g1, taxon].dropna(),
                            data.data.loc[g2, taxon].dropna(),
                        )
