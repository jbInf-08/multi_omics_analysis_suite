...
def model_to_dataset_type(dataset) -> DatasetType:
    ...
    return DatasetType(
        ...
        omics_type=(
            dataset.omics_type.value
            if hasattr(dataset.omics_type, "value")
            else str(dataset.omics_type)
        ),
...
async def get_user(id: str) -> UserType | None:
    """Get user by ID."""
    from backend.app.models.user import User

    async for session in get_async_session():
        try:
            result = await session.execute(select(User).where(User.id == UUID(id)))
...
async def get_project(id: str) -> ProjectType | None:
    """Get project by ID."""
    from backend.app.models.project import Project

    async for session in get_async_session():
        try:
            result = await session.execute(select(Project).where(Project.id == UUID(id)))
...
async def get_dataset(id: str) -> DatasetType | None:
    """Get dataset by ID."""
    from backend.app.models.dataset import Dataset

    async for session in get_async_session():
        try:
            result = await session.execute(select(Dataset).where(Dataset.id == UUID(id)))
...
async def get_analysis(id: str) -> AnalysisType | None:
    """Get analysis by ID."""
    from backend.app.models.analysis import Analysis

    async for session in get_async_session():
        try:
            result = await session.execute(select(Analysis).where(Analysis.id == UUID(id)))
