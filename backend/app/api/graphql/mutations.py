...
class Mutation:
    ...
    async def update_project(
        ...
        async for session in get_async_session():
            try:
                result = await session.execute(select(Project).where(Project.id == UUID(str(id))))
    ...
    async def delete_dataset(self, id: strawberry.ID) -> bool:
        ...
        async for session in get_async_session():
            try:
                result = await session.execute(select(Dataset).where(Dataset.id == UUID(str(id))))
    ...
    async def cancel_analysis(self, id: strawberry.ID) -> AnalysisType:
        ...
        async for session in get_async_session():
            try:
                result = await session.execute(select(Analysis).where(Analysis.id == UUID(str(id))))
                ...
                if analysis.status not in [AnalysisStatus.PENDING, AnalysisStatus.RUNNING]:
                    raise Exception(
                        f"Analysis cannot be cancelled (status: {analysis.status.value})"
                    )
