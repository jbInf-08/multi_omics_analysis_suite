...
def run_quality_control(
    ...
    try:
        ...
        self.update_state(state="PROGRESS", meta={"progress": 0.2, "step": "Checking missing values"})
        ...
        if samples_high_missing > 0:
            qc_results["warnings"].append(f"{samples_high_missing} samples have >{missing_threshold*100}% missing values")
            qc_results["recommendations"].append(f"Consider removing samples with high missing rates")
        
        if features_high_missing > 0:
            qc_results["warnings"].append(f"{features_high_missing} features have >{missing_threshold*100}% missing values")
            qc_results["recommendations"].append(f"Consider removing features with high missing rates")
        ...
        if not numeric_df.empty:
            ...
            if outlier_method == "zscore":
                ...
                z_scores = np.abs(stats.zscore(numeric_df.fillna(numeric_df.median()), nan_policy="omit"))
        ...
        self.update_state(state="PROGRESS", meta={"progress": 0.8, "step": "Computing distribution metrics"})
        ...
        if not numeric_df.empty:
            ...
            if highly_skewed > n_features * 0.3:
                qc_results["recommendations"].append("Consider log-transformation for highly skewed features")
...
def normalize_dataset(
    ...
    try:
        ...
        try:
            ...
            df = pd.read_parquet(dataset.storage_path)
            original_path = dataset.storage_path
        finally:
            session.close()
        ...
        self.update_state(state="PROGRESS", meta={"progress": 0.2, "step": f"Applying {method} normalization"})
        ...
        if method == "zscore":
            ...
            normalized_df = pd.DataFrame(normalized, index=numeric_df.index, columns=numeric_df.columns)
        ...
        self.update_state(state="PROGRESS", meta={"progress": 0.6, "step": "Saving normalized data"})
        ...
        try:
            ...
            if dataset:
                ...
                dataset.preprocessing_applied = dataset.preprocessing_applied + [f"normalized_{method}"]
                dataset.dataset_metadata = {
                    **dataset.dataset_metadata,
                    "normalization": {
                        "method": method,
...
def cleanup_old_results(
    ...
    try:
        ...
        try:
            from backend.app.models.analysis import AnalysisResult, Analysis, AnalysisStatus
            
            # Find old completed/failed analyses
            old_analyses = session.query(Analysis).filter(
...
def batch_import_datasets(
    ...
    try:
        ...
        try:
            ...
            project = None
            if project_id:
                project = session.query(Project).filter(Project.id == UUID(project_id)).first()
