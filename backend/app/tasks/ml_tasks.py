...
def train_model(
    ...
    try:
        ...
        for i, dataset_id in enumerate(dataset_ids):
            ...
            self.update_state(state="PROGRESS", meta={"progress": progress, "step": f"Loaded {i+1}/{len(dataset_ids)}"})
        ...
        self.update_state(state="PROGRESS", meta={"progress": 0.3, "step": f"Training {model_type}"})
...
def run_automl(
    ...
    try:
        ...
        best_results = {"score": float("-inf"), "model_type": None, "params": None}
        all_trials = []
        ...
        def callback(study, trial):
            ...
            self.update_state(state="PROGRESS", meta={
                "progress": progress, 
                "step": f"Trial {trials_completed[0]}/{n_trials}"
            })
...
def generate_shap_explanations(
    ...
    try:
        ...
        return {
            ...
            "expected_value": float(explainer.expected_value) if hasattr(explainer, "expected_value") and not isinstance(explainer.expected_value, list) else None,
        }
...
def train_gnn_model(
    ...
    try:
        ...
        self.update_state(state="PROGRESS", meta={"progress": 0.2, "step": "Preparing PyTorch data"})
        ...
        class GNN(torch.nn.Module):
            def __init__(self, in_channels, hidden_channels, out_channels, num_layers, dropout, conv_type):
                super().__init__()
                ...
                if conv_type == "gcn":
                    ...
                    Conv = lambda in_c, out_c: GATConv(in_c, out_c, heads=4, concat=False)
        ...
        gnn_model = GNN(n_features, hidden_channels, out_channels, num_layers, dropout, model_type).to(device)
        ...
        if y is not None and hasattr(data, 'test_mask'):
            ...
            with torch.no_grad():
                ...
                metrics["best_val_accuracy"] = max(history["val_acc"]) if history["val_acc"] else None
