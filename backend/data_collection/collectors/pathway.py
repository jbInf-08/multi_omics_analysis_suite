        ...
        try:
            if genes:
                for gene in genes[:20]:
                    annotations = await self._get("bioentity/gene", params={
                        "id": gene,
                        "rows": 100,
                    })
...
class WikiPathwaysCollector(BaseCollector):
    ...
    async def collect(
        ...
        **kwargs
        ...
        try:
            # List pathways
            pathways = await self._get("listPathways", params={
                "organism": organism,
                "format": "json",
            })
            ...
            if pathway_ids:
                for pid in pathway_ids[:10]:
                    pathway = await self._get("getPathway", params={
                        "pwId": pid,
                        "format": "json",
                    })
                    collected_data[pid] = pathway
            ...
            if genes:
                for gene in genes[:10]:
                    results = await self._get("findPathwaysByXref", params={
                        "ids": gene,
                        "codes": "H",
                        "format": "json",
                    })
