        ...
        **kwargs,
        ...
        if not self.has_api_key:
            return self._create_result(
                success=False,
                errors=[
                    "COSMIC API key not configured. Set COSMIC_API_KEY and COSMIC_EMAIL environment variables."
                ],
            ...
            if genes:
                for gene in genes[:20]:
                    try:
                        ...
                        variants_data = await self._get(
                            "variants/lookup", params={"hugoSymbol": gene}
                        )
...
class CIViCCollector(BaseCollector):
    ...
    async def collect(
        self, genes: list[str] | None = None, diseases: list[str] | None = None, **kwargs
    ) -> CollectionResult:
...
class GnomADCollector(BaseCollector):
    ...
    async def collect(
        self, genes: list[str] | None = None, variants: list[str] | None = None, **kwargs
    ) -> CollectionResult:
            ...
            if genes:
                for gene in genes[:10]:
                    ...
                    result = await self._post(
                        "", json_data={"query": query, "variables": {"geneSymbol": gene}}
                    )
...
class DbSNPCollector(BaseCollector):
    ...
    async def collect(
        self, rsids: list[str] | None = None, genes: list[str] | None = None, **kwargs
    ) -> CollectionResult:
