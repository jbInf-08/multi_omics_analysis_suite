...
class TCGACollector(BaseCollector):
    ...
    async def _query_gene_expression(self, genes: list[str], cancer_type: str | None) -> dict:
...
class GDCCollector(BaseCollector):
    ...
    async def collect(
        self, project: str | None = None, data_category: str | None = None, **kwargs
    ) -> CollectionResult:
        """Collect GDC data."""
        start_time = utc_now()
...
class ICGCCollector(BaseCollector):
    ...
    async def collect(
        self, cancer_type: str | None = None, donor_ids: list[str] | None = None, **kwargs
    ) -> CollectionResult:
        """Collect ICGC data."""
        start_time = utc_now()
...
class ENCODECollector(BaseCollector):
    ...
    async def collect(
        self, assay_type: str | None = None, biosample: str | None = None, **kwargs
    ) -> CollectionResult:
        """Collect ENCODE data."""
        start_time = utc_now()
...
class GTExCollector(BaseCollector):
    ...
    async def collect(
        self, genes: list[str] | None = None, tissues: list[str] | None = None, **kwargs
    ) -> CollectionResult:
...
class EnsemblCollector(BaseCollector):
    ...
    async def collect(
        self, genes: list[str] | None = None, species: str = "human", **kwargs
    ) -> CollectionResult:
            ...
            if genes:
                for gene in genes[:20]:
                    gene_data = await self._get(
                        f"lookup/symbol/{species}/{gene}", params={"expand": 1}
                    )
