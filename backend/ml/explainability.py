...
class SHAPExplainer:
    ...
    def plot_summary(self, X: Union[np.ndarray, pd.DataFrame], feature_names: Optional[List[str]] = None):
...
class LIMEExplainer:
    ...
    def explain(
        ...
        feature_importance = {
            f: np.mean(weights) if weights else 0
            for f, weights in feature_weights.items()
        }
