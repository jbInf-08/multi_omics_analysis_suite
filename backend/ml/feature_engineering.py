...
class StatisticalFeatureExtractor:
    ...
    def fit_transform(self, X: np.ndarray) -> Tuple[np.ndarray, List[str]]:
        ...
        if self.include_derived:
            ...
            # Range
            range_val = np.max(X, axis=self.axis, keepdims=True) - np.min(X, axis=self.axis, keepdims=True)
            features.append(range_val)
            names.append("stat_range")
            
            # IQR
            iqr = np.percentile(X, 75, axis=self.axis, keepdims=True) - np.percentile(X, 25, axis=self.axis, keepdims=True)
            features.append(iqr)
...
class InteractionFeatureExtractor:
    ...
    def fit_transform(
        ...
        if self.interaction_only:
            ...
            poly_names = [n for n, m in zip(poly_names, interaction_mask) if m]
...
class AdvancedFeatureEngineer:
    ...
    def fit_transform(
        ...
        feature_types = {name: FeatureType.DOMAIN for name in all_names}
        ...
        if self.include_graph and edge_index is not None:
            ...
            graph_features, graph_names = self.graph_extractor.fit_transform(
