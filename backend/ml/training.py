    ...
    def train_with_cv(
        ...
        for fold, (train_idx, val_idx) in enumerate(cv.split(X_arr, y_arr)):
            ...
            if model.model_type == "classification":
                metrics = self._compute_classification_metrics(y_val, y_pred, model.predict_proba(X_val))
            else:
        ...
        model.metrics = ModelMetrics(**{k: v["mean"] for k, v in cv_results.items() if isinstance(v, dict)})
