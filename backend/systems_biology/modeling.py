...
def _rate_law_to_mathml(rate_law: str, id_for_name: Dict[str, str]) -> Optional[str]:
    """
    ...
    def walk(node: ast.AST) -> Optional[str]:
...
class ODEModel:
    ...
    def derivatives(
        ...
        for i, reaction in enumerate(self.reactions):
            # Calculate rate
            if self._compiled_rates:
                rate = self._compiled_rates[i](y, params)
            else:
                rate = 0.0
    ...
    def to_sbml(self) -> str:
        ...
        for p in self.parameters.values():
            lines.append(
                f'<parameter id="{sid(p.name)}" value="{p.value:g}" constant="true"/>'
            )
...
class SteadyStateAnalysis:
    ...
    def find_steady_state(
        ...
        if initial_guess is None:
            y = self.model.get_initial_state()
        else:
...
class ParameterEstimation:
    ...
    def fit(
        ...
        if parameter_bounds is None:
            parameter_bounds = {name: (0.001 * params[name], 100 * params[name]) 
                              for name in param_names}
