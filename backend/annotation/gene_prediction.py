...
class AugustusPredictor(GenePredictor):
    ...
    def predict(self, sequence: str, contig_id: str = "contig") -> list[GenePrediction]:
        ...
        strands = (
            ["+"]
            if self.search_strand == "+"
            else (["-"] if self.search_strand == "-" else ["+", "-"])
        )
        ...
        for strand in strands:
            ...
            for gene in genes:
                if strand == "-":
                    ...
                    gene["exons"] = [
                        (len(sequence) - e[1], len(sequence) - e[0])
                                ...
                                genes.append(
                                    {
                                        ...
                                        "score": self._score_gene(
                                            sequence, start, stop + 3, [(donor, acceptor)]
                                        ),
                                    }
                                )
    ...
    def predict(self, sequence: str, contig_id: str = "contig") -> list[GenePrediction]:
        ...
        if not self.trained:
            ...
            orf_finder = ORFFinder(
                min_length=self.min_length if hasattr(self, "min_length") else 100
            )
