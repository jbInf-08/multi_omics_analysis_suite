    ...
    def cluster_orthologs(
        ...
        for gene_key in gene_to_orthologs:
            ...
            if component:
                ...
                genomes_in_group = set(g[0] for g in component)
...
class SyntenyAnalyzer:
    ...
    def find_synteny(
        self,
        orthologs: List[Ortholog],
        gene_positions: Dict[str, Dict[str, Tuple[str, int, int, str]]],  # genome -> gene -> (contig, start, end, strand)
    ) -> List[SyntenyBlock]:
...
class GeneClusterFinder:
    ...
    def _extend_cluster(
        ...
        cluster_start = min(g.start for g in core_genes)
        cluster_end = max(g.end for g in core_genes)
