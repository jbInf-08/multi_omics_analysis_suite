...
class RIntegrationManager:
    ...
    def load_package(self, package: Union[str, RPackage]) -> Optional[Any]:
        """
        Load an R package.
...
class DESeq2Analyzer:
    ...
    def __init__(
        ...
        self.results_: Optional[DEAnalysisResult] = None
    ...
    def _run_fallback(
        self, counts: pd.DataFrame, metadata: pd.DataFrame
    ) -> DEAnalysisResult:
        ...
        try:
            ...
            results_df.columns = ["gene", "baseMean", "log2FoldChange", 
    ...
    def _process_results(
        self, results_df: pd.DataFrame, method: str
    ) -> DEAnalysisResult:
        ...
        upregulated = significant[
            significant["log2_fold_change"] > self.lfc_threshold
        ]["gene"].tolist()
...
class LimmaAnalyzer:
    ...
    def run(
        ...
        contrast_matrix: Optional[pd.DataFrame] = None,
    ...
    def run_from_groups(
        ...
        for sample in expression.columns:
            group = groups[sample] if sample in groups.index else groups.iloc[
                list(expression.columns).index(sample)
            ]
