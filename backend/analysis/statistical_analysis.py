...
class TestResult:
    ...
    mean_group1: float | None = None
    mean_group2: float | None = None
    std_group1: float | None = None
    std_group2: float | None = None
    n_group1: int | None = None
...
class EffectSizeCalculator:
    ...
    @staticmethod
    def eta_squared(
        groups: list[np.ndarray], ss_between: float | None = None, ss_total: float | None = None
    ) -> float:
...
class MultipleTestingCorrection:
    ...
    def correct(
        p_values: np.ndarray,
        method: str | MultipleTestingMethod = MultipleTestingMethod.FDR_BH,
        alpha: float = 0.05,
    ...
    def benjamini_hochberg(
        ...
        adjusted = np.zeros(n)
        for i in range(n):
            adjusted[sorted_indices[i]] = min(sorted_pvals[i] * n / (i + 1), 1.0)
    ...
    def q_value(p_values: np.ndarray) -> np.ndarray:
        ...
        pi0_estimates = []
        for lam in lambdas:
            pi0_estimates.append(np.sum(sorted_pvals > lam) / (n * (1 - lam)))
        pi0 = min(1.0, np.median(pi0_estimates))

        # Compute q-values
        q_values = np.zeros(n)
        for i in range(n):
            q_values[sorted_indices[i]] = min(pi0 * sorted_pvals[i] * n / (i + 1), 1.0)
...
class StatisticalAnalysisPipeline:
    ...
    def __init__(
        self,
        # Test selection
        test: StatisticalTest | None = None,
        ...
        self.results_: StatisticalAnalysisResult | None = None
    ...
    def _test_feature(
        ...
        # Split by groups
        group_data = [feature_data[groups == g] for g in unique_groups]
        ...
        try:
            if len(group_data) == 2:
                if self.effect_size_type == "cohens_d":
                    ...
                    delta, _ = EffectSizeCalculator.cliffs_delta(group_data[0], group_data[1])
    ...
    def get_significant_features(
        self, adjusted: bool = True, alpha: float | None = None
    ) -> list[str]:
        ...
        if adjusted:
            return [
                r.feature
                for r in self.results_.results
                if r.adjusted_p_value is not None and r.adjusted_p_value < alpha
            ]
        else:
            return [r.feature for r in self.results_.results if r.p_value < alpha]
...
def differential_expression_analysis(
    ...
    pipeline = StatisticalAnalysisPipeline(
        test=(
            None
            if method == "auto"
            else (StatisticalTest.TTEST_IND if method == "ttest" else StatisticalTest.MANN_WHITNEY)
        ),
    ...
    results = pipeline.analyze(
        expression_matrix.T, condition_labels.values, feature_names=expression_matrix.index.tolist()
    )

    # Add DEG classification
    summary = results.summary.copy()
    summary["is_deg"] = (summary["adjusted_p_value"] < fdr_threshold) & (
        summary["log2_fold_change"].abs() > log2fc_threshold
    )
    summary["deg_class"] = "not_significant"
    summary.loc[
        summary["is_deg"] & (summary["log2_fold_change"] > log2fc_threshold), "deg_class"
    ] = "upregulated"
    summary.loc[
        summary["is_deg"] & (summary["log2_fold_change"] < -log2fc_threshold), "deg_class"
    ] = "downregulated"
