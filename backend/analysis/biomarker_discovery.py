...
class StabilitySelector:
    ...
    def __init__(
        ...
        self.selection_probabilities_: np.ndarray | None = None
        self.selected_features_: np.ndarray | None = None
        self.stability_scores_: dict[str, float] | None = None
    ...
    def fit(self, X: np.ndarray, y: np.ndarray) -> "StabilitySelector":
        ...
        self.stability_scores_ = {
            f"feature_{i}": prob for i, prob in enumerate(self.selection_probabilities_)
        }

        logger.info(f"Selected {len(self.selected_features_)} stable features")
        return self

    def _apply_base_selector(self, X: np.ndarray, y: np.ndarray, n_features: int) -> np.ndarray:
        ...
        try:
            if self.base_selector == "lasso":
                ...
                model = LogisticRegressionCV(
                    cv=3, penalty="l1", solver="saga", random_state=self.random_state, max_iter=5000
                )
                model.fit(X, y)
...
class ConsensusScorer:
    ...
    def __init__(
        self,
        methods: list[FeatureSelectionMethod] | None = None,
        ...
        self.method_scores_: dict[str, np.ndarray] | None = None
        self.consensus_scores_: np.ndarray | None = None
        self.feature_rankings_: np.ndarray | None = None
    ...
    def _compute_consensus(self) -> None:
        ...
        for i, (method_name, scores) in enumerate(self.method_scores_.items()):
            if self.normalize_scores and scores.max() > 0:
...
class BiomarkerDiscoveryPipeline:
    ...
    def __init__(
        ...
        # Results
        self.results_: BiomarkerDiscoveryResult | None = None
    ...
    def fit(
        ...
        candidates = self._create_candidates(
            X, y, feature_names, self.consensus_scorer_.consensus_scores_, stability_scores
        )
    ...
    def _create_candidates(
        ...
        for i, name in enumerate(feature_names):
            ...
            selection_count = sum(
                1
                for scores in self.consensus_scorer_.method_scores_.values()
    ...
    def _compute_additional_stats(
        ...
        [c.feature_name for c in candidates]
        feature_idx = {name: i for i, name in enumerate(self.consensus_scorer_.feature_names_)}
    ...
    def transform(self, X: np.ndarray | pd.DataFrame) -> np.ndarray:
        ...
        if isinstance(X, pd.DataFrame):
            ...
            selected_indices = [
                feature_names.index(c.feature_name) for c in self.results_.candidates
            ]
