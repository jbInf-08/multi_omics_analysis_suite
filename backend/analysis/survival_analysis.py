...
class KaplanMeierResult:
    ...
    median_ci_lower: float | None
    median_ci_upper: float | None
    survival_function: pd.DataFrame
    confidence_interval: pd.DataFrame
    at_risk_counts: pd.Series
    restricted_mean: float | None = None
    restricted_mean_se: float | None = None
...
class LogRankResult:
    ...
    is_significant: bool
    group_comparisons: dict[str, float] | None = None
...
class SurvivalAnalysisResult:
    ...
    cox_results: list[CoxPHResult] | None
    logrank_result: LogRankResult | None
    concordance_index: float | None
    concordance_ci: tuple[float, float] | None
    n_total: int
    n_events: int
...
class KaplanMeierEstimator:
    ...
    def __init__(
        ...
        self.result_: KaplanMeierResult | None = None
...
class CoxProportionalHazards:
    ...
    def __init__(
        ...
        self.results_: list[CoxPHResult] | None = None
    ...
    def predict_survival_function(
        self, data: pd.DataFrame, times: np.ndarray | None = None
    ) -> pd.DataFrame:
        ...
        for i, g1 in enumerate(unique_groups):
            for g2 in unique_groups[i + 1 :]:
                ...
                pw_result = logrank_test(
                    durations[mask1], durations[mask2], events[mask1], events[mask2]
                )
...
class SurvivalAnalysisPipeline:
    ...
    def __init__(
        ...
        self.results_: SurvivalAnalysisResult | None = None
    ...
    def analyze(
        ...
        if self.run_logrank and group_col is not None:
            ...
            logrank_result = multivariate_logrank_comparison(durations, events, groups, self.alpha)
...
def biomarker_survival_analysis(
    ...
    output = {
        ...
        "logrank_significant": (
            results.logrank_result.is_significant if results.logrank_result else None
        ),
