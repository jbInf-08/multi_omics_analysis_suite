...
ATOMIC_MASSES = {
    ...
    'Na': 22.990, 'K': 39.098, 'Ca': 40.078, 'Mg': 24.305, 'Zn': 65.38,
    'Fe': 55.845, 'Cu': 63.546,
}
...
class Molecule:
    ...
    def get_bond(self, atom1_idx: int, atom2_idx: int) -> Optional[Bond]:
    ...
    def from_pdb(cls, pdb_content: str) -> "Molecule":
        ...
        for line in pdb_content.strip().split('\n'):
            if line.startswith(('ATOM', 'HETATM')):
                ...
                atoms = [int(line[i:i+5].strip()) - 1 for i in range(6, len(line), 5) if line[i:i+5].strip()]
                if len(atoms) >= 2:
