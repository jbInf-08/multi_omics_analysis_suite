...
class AlignmentResult:
    ...
    cigar: str
    query_sequence: str
    query_qualities: Optional[str] = None
...
class BurrowsWheelerAligner(Aligner):
    ...
    def align(self, query: str, query_name: str = "read") -> List[AlignmentResult]:
        ...
        if positions:
            ...
            for pos in positions[:10]:  # Limit to 10 hits
                results.append(self._create_alignment(
                    query_name, query, pos, is_exact=True
                ))
            return results
        ...
        if positions:
            ...
            for pos, mismatches in positions[:10]:
                results.append(self._create_alignment(
                    query_name, query, pos, mismatches=mismatches
                ))
            return results
    ...
    def _exact_match(self, pattern: str) -> List[int]:
        ...
        for char in reversed(pattern):
            ...
            bottom = self.c[char] + (self.occ[char][bottom] if bottom < len(self.occ[char]) else self.occ[char][-1])
        ...
        for i in range(max_mismatches + 1):
            ...
            for pos in seed_positions:
                ...
                if 0 <= ref_start <= len(self.reference) - len(pattern):
                    ...
                    mismatches = sum(1 for a, b in zip(pattern, ref_region) if a != b)
...
class MiniMap2Aligner(Aligner):
    ...
    def align(self, query: str, query_name: str = "read") -> List[AlignmentResult]:
        ...
        return results if results else [AlignmentResult(
            query_name=query_name,
            reference_name="*",
            reference_start=0,
            mapping_quality=0,
    ...
    def _extend_chain(self, chain: List[Tuple[int, int]], query: str) -> Optional[Dict]:
...
class SplicedAligner(Aligner):
    ...
    def __init__(
        self,
        known_junctions: Optional[List[Tuple[int, int]]] = None,
        **kwargs,
    ):
        super().__init__(**kwargs)
        ...
        for r1 in results1:
            ...
            for r2 in results2:
                ...
                if 50 < intron_end - intron_start < 500000:  # Valid intron size
                    ...
                    if (self.reference[intron_start:intron_start + 2] == "GT" and
                        self.reference[intron_end - 2:intron_end] == "AG"):
