...
class PileupGenerator:
    ...
    def generate(
        ...
        # Filter alignments
        filtered = [
            a for a in alignments if a.is_mapped and a.mapping_quality >= self.min_mapping_quality
        ]
    ...
    def _get_query_position(self, alignment: "AlignmentResult", ref_pos: int) -> int | None:
...
class DuplicateMarker:
    ...
    def mark_duplicates(
        ...
        for i, aln in enumerate(alignments):
            ...
            pos = aln.reference_end if aln.is_reverse else aln.reference_start
...
class BaseRecalibrator:
    ...
    def build_recalibration_table(
        ...
        alignments: list["AlignmentResult"],
        reference: str,
        known_sites: list[tuple[int, str, str]] | None = None,
    ):
        """Build recalibration table from alignments."""
        known_positions = set()
        if known_sites:
            for pos, _ref, _alt in known_sites:
