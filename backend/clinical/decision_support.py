...
class ClinicalGuidelines:
    ...
    def get_guidelines_for_biomarker(
        self, tumor_type: str, biomarker: str
    ) -> Optional[Dict]:
...
class ClinicalDecisionSupport:
    ...
    def generate_recommendations(
        ...
        tumor_type: str,
        molecular_profile: Dict[str, Any],
        patient_characteristics: Optional[Dict] = None,
        ...
        for mutation in mutations:
            ...
            guideline = self.guidelines.get_guidelines_for_biomarker(
                tumor_type, biomarker_key
            )
            
            if guideline:
                rec = self._create_recommendation(
                    guideline, gene, variant, tumor_type
                )
        ...
        for biomarker, value in biomarkers.items():
            if self._is_actionable_biomarker(biomarker, value, tumor_type):
                guideline = self.guidelines.get_guidelines_for_biomarker(
                    tumor_type, biomarker
                )
                if guideline:
                    rec = self._create_recommendation(
                        guideline, biomarker, str(value), tumor_type
                    )
                    recommendations.append(rec)
        
        # Sort by evidence level
        recommendations.sort(
            key=lambda r: r.strength.value,
            reverse=False
        )
    ...
    def _is_actionable_biomarker(
        self, biomarker: str, value: Any, tumor_type: str
    ) -> bool:
        """Check if a biomarker is actionable."""
        actionable_biomarkers = {
            "PD_L1_high": lambda v: v and (v == "high" or (isinstance(v, (int, float)) and v >= 50)),
    ...
    def assess_risk(
        ...
        stage: str,
        molecular_profile: Dict[str, Any],
        clinical_factors: Optional[Dict] = None,
    ) -> RiskAssessment:
        """
