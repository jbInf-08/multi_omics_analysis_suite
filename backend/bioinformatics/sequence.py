...
class Sequence(ABC):
    ...
    def find(self, pattern: str, start: int = 0, end: Optional[int] = None) -> int:
    ...
    def melting_temperature(self, method: str = "basic") -> float:
        ...
        if method == "basic":
            ...
            if len(self._sequence) < 14:
                return 2 * (self._sequence.count('A') + self._sequence.count('T')) + \
                       4 * (self._sequence.count('G') + self._sequence.count('C'))
            else:
                # Wallace rule
                return 64.9 + 41 * (self._sequence.count('G') + self._sequence.count('C') - 16.4) / len(self._sequence)
        ...
        for i in range(len(self._sequence) - min_stem * 2 - min_loop):
            for stem_len in range(min_stem, 15):
                for loop_len in range(min_loop, max_loop + 1):
                    ...
                    rev_stem2 = stem2[::-1]
                    matches = sum(1 for a, b in zip(stem1, rev_stem2)
                                  if (a, b) in [('A', 'U'), ('U', 'A'), ('G', 'C'), ('C', 'G'), ('G', 'U'), ('U', 'G')])
...
class ProteinSequence(Sequence):
    ...
    def secondary_structure_propensity(self) -> Dict[str, float]:
        ...
        helix_score = sum(helix_propensity.get(aa, 1.0) for aa in self._sequence) / len(self._sequence)
        sheet_score = sum(sheet_propensity.get(aa, 1.0) for aa in self._sequence) / len(self._sequence)
...
class SequenceCollection:
    ...
    def filter_by_length(self, min_length: int = 0, max_length: Optional[int] = None) -> "SequenceCollection":
    ...
    def consensus(self) -> Optional[Sequence]:
