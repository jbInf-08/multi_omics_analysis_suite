...
from .sequence import STANDARD_CODON_TABLE, DNA_COMPLEMENT
...
def sliding_window(
    ...
    for i in range(0, len(sequence) - window_size + 1, step):
        ...
        if function == "gc":
            ...
            total = len(window)
            value = -sum((c / total) * math.log2(c / total) 
                        for c in freqs.values() if c > 0)
        
        elif function == "complexity":
            # Linguistic complexity (unique k-mers / possible k-mers)
            k = 3
            kmers = set(window[j:j + k] for j in range(len(window) - k + 1))
