...
from .sequence import DNASequence, ProteinSequence, RNASequence, SequenceCollection, SequenceQuality
...
class SAMAlignment:
    ...
    def get_aligned_pairs(self) -> list[tuple[int | None, int | None]]:
...
class VCFVariant:
    ...
    def get_genotype(self, sample: str) -> str | None:
...
class GenBankFeature:
    ...
    def get_qualifier(self, key: str) -> str | None:
...
class FileParser(ABC):
    ...
    @abstractmethod
    def parse(self, filepath: str | Path) -> Iterator:
        """Parse file and yield records."""
        pass

    def _open_file(self, filepath: str | Path) -> TextIO:
...
class FastaParser(FileParser):
    ...
    def parse(self, filepath: str | Path) -> Iterator[FastaRecord]:
    ...
    def parse_to_collection(self, filepath: str | Path) -> SequenceCollection:
    ...
    @staticmethod
    def write(records: list[FastaRecord], filepath: str | Path, line_width: int = 60):
...
class FastqParser(FileParser):
    ...
    def parse(self, filepath: str | Path) -> Iterator[FastqRecord]:
    ...
    def parse_paired(
        ...
        yield from zip(parser1, parser2, strict=False)

    @staticmethod
    def write(records: list[FastqRecord], filepath: str | Path):
...
class GFFParser(FileParser):
    ...
    def parse(self, filepath: str | Path) -> Iterator[GFFFeature]:
    ...
    def parse_genes(self, filepath: str | Path) -> dict[str, list[GFFFeature]]:
...
class BEDParser(FileParser):
    ...
    def parse(self, filepath: str | Path) -> Iterator[BEDRecord]:
    ...
    @staticmethod
    def write(records: list[BEDRecord], filepath: str | Path):
...
class SAMParser(FileParser):
    ...
    def parse(self, filepath: str | Path) -> Iterator[SAMAlignment]:
...
class VCFParser(FileParser):
    ...
    def parse(self, filepath: str | Path) -> Iterator[VCFVariant]:
            ...
            for line in f:
                ...
                for i, sample_name in enumerate(self.header["samples"]):
                    if len(fields) > 9 + i:
                        ...
                        samples[sample_name] = dict(zip(format_fields, sample_values, strict=False))
...
class GenBankParser(FileParser):
    ...
    def parse(self, filepath: str | Path) -> Iterator[GenBankRecord]:
    ...
    def _parse_record(self, text: str) -> GenBankRecord | None:
        ...
        for line in lines:
            ...
            if line.startswith("LOCUS"):
                ...
                locus_info = {
                    ...
                    "topology": (
                        parts[5]
                        if len(parts) > 5 and parts[5] in ["linear", "circular"]
                        else "linear"
                    ),
